...
function calcBadges(playerId, matches) {
  ...
  let hatTricks = 0, lossTricks = 0, cleanWins = 0, cleanLosses = 0, cleanHits = 0;
  ...
  for (const m of pm) {
    ...
    if ((m.cleanHitBy || []).includes(playerId)) cleanHits++;
  }
  return { hatTricks, lossTricks, cleanWins, cleanLosses, cleanHits };
}
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
                        ...
                        {badges.lossTricks > 0 && (
                          <span style={{ position:"relative", display:"inline-flex" }} onMouseEnter={() => setTooltip(`${p.id}-loss-trick`)} onMouseLeave={() => setTooltip(null)}>
                            <span style={{ fontSize:11, borderRadius:20, padding:"1px 6px", fontWeight:600, background:"#fef2f2", color:"#b91c1c", border:"0.5px solid #fca5a5" }}>{badges.lossTricks} 😢</span>
                            {tooltip === `${p.id}-loss-trick` && <span style={{ position:"absolute", bottom:"calc(100% + 6px)", left:"50%", transform:"translateX(-50%)", background:"#1f2937", color:"#fff", fontSize:11, borderRadius:8, padding:"6px 10px", whiteSpace:"nowrap", zIndex:100, lineHeight:1.5, textAlign:"center", pointerEvents:"none", boxShadow:"0 2px 8px rgba(0,0,0,0.15)" }}>😢 Loss-trick!<br/>{badges.lossTricks === 1 ? "1 loss-trick" : `${badges.lossTricks} loss-tricks`}<span style={{ position:"absolute", top:"100%", left:"50%", transform:"translateX(-50%)", borderWidth:5, borderStyle:"solid", borderColor:"#1f2937 transparent transparent transparent" }}/></span>}
                          </span>
                        )}
                        {badges.cleanWins > 0 && (
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                          ...
                          {badges.lossTricks > 0 && (
                            <span style={{ position:"relative", display:"inline-flex", flexShrink:0 }} onMouseEnter={() => setTooltip(`${p.id}-loss-trick`)} onMouseLeave={() => setTooltip(null)}>
                              <span style={{ display:"inline-flex", alignItems:"center", gap:3, fontSize:11, borderRadius:20, padding:"2px 8px", fontWeight:600, background:"#fef2f2", color:"#b91c1c", border:"0.5px solid #fca5a5" }}>{badges.lossTricks} 😢</span>
                              {tooltip === `${p.id}-loss-trick` && <span style={{ position:"absolute", bottom:"calc(100% + 6px)", left:"50%", transform:"translateX(-50%)", background:"#1f2937", color:"#fff", fontSize:11, borderRadius:8, padding:"6px 10px", whiteSpace:"nowrap", zIndex:100, lineHeight:1.5, textAlign:"center", pointerEvents:"none", boxShadow:"0 2px 8px rgba(0,0,0,0.15)" }}>😢 Loss-trick!<br/>{badges.lossTricks === 1 ? "1 loss-trick" : `${badges.lossTricks} loss-tricks`}<span style={{ position:"absolute", top:"100%", left:"50%", transform:"translateX(-50%)", borderWidth:5, borderStyle:"solid", borderColor:"#1f2937 transparent transparent transparent" }}/></span>}
                            </span>
                          )}
                          {badges.cleanWins > 0 && (
...
function NewMatch({ players, onSave, prefilled }) {
  ...
  const [cleanHitBy, setCleanHitBy] = useState([]);

  function toggleCleanHit(id) {
    setCleanHitBy(prev => prev.includes(id) ? prev.filter(x => x !== id) : [...prev, id]);
  }
  ...
  function changeFmt(f) { setFmt(f); setT1(f === "1v1" ? [""] : ["", ""]); setT2(f === "1v1" ? [""] : ["", ""]); setWinner(null); setWinnerScore(""); setLoserScore(""); setScoreError(""); setCleanHitBy([]); }
  ...
  return (
    <div style={{ maxWidth: 480 }}>
      ...
      {t1ok && t2ok && (
        <div style={{ marginBottom: "1.25rem" }}>
          <p className="section-label">💥 Clean Hit (optional)</p>
          <p style={{ fontSize: 11, color: "var(--text-muted)", marginTop: -6, marginBottom: 8 }}>
            Select anyone who pocketed all their pieces in one turn. Badge only, no extra points.
          </p>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
            {[...t1, ...t2].filter(Boolean).map(id => {
              const selected = cleanHitBy.includes(id);
              return (
                <button key={id} type="button" onClick={() => toggleCleanHit(id)}
                  style={{
                    display: "inline-flex", alignItems: "center", gap: 6,
                    padding: "6px 12px", borderRadius: 20, fontSize: 13, fontWeight: 600,
                    cursor: "pointer", fontFamily: "inherit",
                    background: selected ? "#fff1f2" : "transparent",
                    color: selected ? "#9f1239" : "var(--text)",
                    border: `1.5px solid ${selected ? "#fb7185" : "var(--border-strong)"}`,
                  }}>
      ...
      {runningMatch && currentUser && (
        <div className="running-banner" style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
            ...
            >
              <span style={{ fontSize: 16, fontWeight: 700, lineHeight: 1 }}>+</span><span style={{ marginLeft: 6 }}>Enter Result</span>
            </button>
          </div>
